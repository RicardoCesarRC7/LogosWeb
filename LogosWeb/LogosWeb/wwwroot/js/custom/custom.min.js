...
(function($){
    ...
    $(document).ready(function() {
        ...
        contact_form.each(function () {
            $(this).find('input,textarea').jqBootstrapValidation({
                ...
                submitSuccess: function ($form, event) {
                    ...
                    var telefone = $('[name="telefone"]', $form).val();
                    var area = $('[name="area"]', $form).val();
                    var curso = $('[name="curso"]', $form).val();
                    var formato = $('[name="formato"]', $form).val();
                    var message = $('[name="message"]', $form).val();

                    $.ajax({
                        type: 'POST',
                        url: window.location.protocol + '//' + window.location.host + '/Service/SendInteresseEmail',
                        dataType: 'json',
                        data: {
                            primeiroNome: name,
                            segundoNome: surname,
                        ...
                        beforeSend: function (result) {
                            $('.form-submit-button', $form).html('Enviando <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>');
                        },
                        success: function (result) {
                            if (result.success) {
                                $('.form-submit-button', $form).html('Enviado <i class="fas fa-check"></i>');
                                $('.title-form-interesse').html('Já recebemos suas respostas!');
                                $form.fadeOut(500);
                            } else {
        ...
        $('#estudio-form').find('input,textarea').jqBootstrapValidation({
            preventSubmit: true,
            submitError: function ($form, event, errors) {
                // additional error messages or events
            },
            submitSuccess: function ($form, event) {
                ...
                var submit = $('submit', $form);
                var ajaxResponse = $('#contact-response');
                let nomeCompleto = $('[name="nome-completo"]', $form).val();
                let email = $('[name="email"]', $form).val();
                let telefone = $('[name="telefone"]', $form).val();
                let forma = $('[name="forma-doacao"]', $form).val() == 'Outra' ? $('#outra-forma-estudio').val() : $('[name="forma-doacao"]', $form).val();
                let valor = $('[name="valor"]', $form).val();
                let valorMensal = $('[name="valor-mensal"]', $form).val();
                let residencial = $('[name="residencial"]', $form).val();
                let cpf = $('[name="cpf"]', $form).val();
                let mensagem = $('[name="mensagem"]', $form).val();

                $.ajax({
                    type: 'POST',
                    url: window.location.protocol + '//' + window.location.host + '/Service/SendEstudioEmail',
                    dataType: 'json',
                    data: {
                        nome: nomeCompleto,
                        email: email,
                    ...
                    cache: false,
                    beforeSend: function (result) {
                        $('#form-submit-button-estudio', $form).html('Enviando <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>');
                    },
                    success: function (result) {
                        if (result.success) {
                            $('#form-submit-button-estudio', $form).html('Enviado <i class="fas fa-check"></i>');
                            $('#title-form-estudio').html('Já recebemos suas respostas! Muito obrigado!');
                            $form.fadeOut(500);
                        } else {
                            $('#form-submit-button-estudio', $form).html('Erro ao enviar. Tente novamente!');
                            $('#title-form-estudio').html('Já recebemos suas respostas! Muito obrigado!');
                        }
                    }
                });
        ...
        $('#form-interesse .area-interesse-form').select2({ theme: 'classic', placeholder: 'Clique para selecionar a área de interesse' });
        $('#form-interesse .curso-interesse-form').select2({ theme: 'classic', placeholder: 'Clique para selecionar o curso de interesse' });
        $('#form-interesse .formato-interesse-form').select2({ theme: 'classic', placeholder: 'Clique para selecionar o formato' });
        ...
        $(window).scroll(function() {
            if ($(this).scrollTop() > 100) {
                $('.form-icon').addClass('form-icon-visible');
                $('.whatsapp-icon').addClass('whatsapp-icon-visible');
                $('.podcast-icon').addClass('podcast-icon-visible');
            } else {
                $('.form-icon').removeClass('form-icon-visible');
                $('.whatsapp-icon').removeClass('whatsapp-icon-visible');
                $('.podcast-icon').removeClass('podcast-icon-visible');
            }
        });

        $('#forma-doacao-estudio').on('change', function () {

            if ($(this).val() == 'Outra') {
                $('#outra-forma-estudio').prop('disabled', null);
            } else {
                $('#outra-forma-estudio').prop('disabled', true);
            }
        });

        $('#hero-sobre-nos-button').on('click', function () {
            window.location = window.location.protocol + '//' + window.location.host + '/Sobre';
        });

        if ($('#estudio-form').length > 0) {
            $('[name="valor"]').mask('000.000.000.000.000,00', {reverse: true});
            $('[name="cpf"]').mask('000.000.000-00', { reverse: true });
        }

        //$('#modal-propagandas').modal('show');
    });
